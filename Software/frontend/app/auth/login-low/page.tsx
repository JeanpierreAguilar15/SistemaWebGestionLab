...
export default function LoginLowPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Llamar al endpoint VULNERABLE
            const response = await fetch(`${API_URL}/auth/login-low`, {
                method: 'POST',
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-lab-primary-50 via-white to-lab-success-50 p-4">
            <div
                className={`w-full max-w-md space-y-6 transition-opacity duration-500 ${mounted ? 'opacity-100' : 'opacity-0'
                    }`}
            >
                {/* Logo y Header */}
                <div className="text-center space-y-2">
                    <div className="flex justify-center mb-4">
                        <div className={`bg-lab-primary-600 text-white rounded-2xl p-4 shadow-lg transition-colors duration-300 ${success ? 'bg-green-500' : ''
                            }`}>
                ...
                {/* Login Card */}
                <Card className={`border-lab-neutral-200 shadow-xl transition-all duration-300 ${shake ? 'animate-shake' : ''
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {/* Mensaje de éxito */}
                            <div className={`overflow-hidden transition-all duration-200 ${success ? 'max-h-20 opacity-100' : 'max-h-0 opacity-0'
                                }`}>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        className={`pl-10 transition-colors duration-200 ${error ? 'border-lab-danger-300' : ''
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        required
                                        disabled={loading || success}
                                        className={`pl-10 pr-10 transition-colors duration-200 ${error ? 'border-lab-danger-300' : ''
                                            } ${success ? 'border-green-300 bg-green-50/30' : ''}`}
                                    />
                                    <svg className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-lab-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <Button
                                type="submit"
                                className={`w-full h-11 text-base font-medium transition-colors duration-200 ${success ? 'bg-green-500 hover:bg-green-600' : ''}`}
                                disabled={loading || success}
                            >
                                {loading ? (
                            ...
                            <div className={`text-center text-sm text-lab-neutral-600 transition-opacity duration-200 ${success ? 'opacity-50' : ''
                                }`}>
